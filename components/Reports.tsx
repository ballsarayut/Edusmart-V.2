...
const Reports: React.FC<ReportsProps> = ({
  ...
  const parentChild = useMemo(() => {
    ...
    return students.find(
      (s) =>
        String(s.id) === String(user.studentId) ||
        String(s.studentId) === String(user.studentId),
    );
  }, [isParent, students, user.studentId]);

  const [activeTab, setActiveTab] = useState<"individual" | "room">(
    initialTab || (isParent ? "individual" : "room"),
  );
  ...
  const availableLevels = useMemo(
    () =>
      Array.from(new Set(students.map((s) => s.level)))
        .map(String)
        .sort(),
    [students],
  );
  ...
  const availableRooms = useMemo(
    () =>
      Array.from(new Set(students.map((s) => s.room)))
        .map(String)
  ...
  const getStudentMetrics = (studentId: string) => {
    ...
    const presents = morningRecords.filter(
      (r) => r.status === "PRESENT",
    ).length;
    ...
    const rate =
      morningRecords.length > 0
        ? Math.round((presents / morningRecords.length) * 100)
        : 0;

    const student = students.find((s) => String(s.id) === String(studentId));
    const config = tuitionConfigs.find(
      (c) => c.level === student?.level && c.department === student?.department,
    );
    ...
    const sermonRecords = studentRecords.filter((r) => r.type === "SERMON");
    ...
    sermonRecords.forEach((r) => {
      if (r.sermonBlock && r.date && r.status === "NOT_RECORDED") {
    ...
    morningRecords.forEach((r) => {
      if (
        ...
        (r.status === "ABSENT" ||
          r.status === "LATE" ||
          r.status === "BUSINESS_LEAVE" ||
          r.status === "SICK_LEAVE")
      ) {
        const matchingSermon = sermonRecords.find((s) => s.date === r.date);
    ...
    const sermonScore = Number(
      Object.values(blockSermonScores)
  ...
  const exportIndividualExcel = () => {
    ...
    XLSX.writeFile(
      wb,
      `รายงานผล_${selectedStudent.studentId}_${selectedStudent.name}.xlsx`,
    );
  };
...
ข้อมูลส่งตรงจากระบบ EduSmart Vocational CMS`;
  ...
  const exportRoomExcel = () => {
    ...
    const summaryData = roomStudents.map((s, idx) => {
      ...
      const b1Sum = Number(
        (
          (metrics.blockAttendance[1] || 0) +
          (metrics.blockSermonScores[1] || 0)
        ).toFixed(2),
      );
      const b2Sum = Number(
        (
          (metrics.blockAttendance[2] || 0) +
          (metrics.blockSermonScores[2] || 0)
        ).toFixed(2),
      );
      const b3Sum = Number(
        (
          (metrics.blockAttendance[3] || 0) +
          (metrics.blockSermonScores[3] || 0)
        ).toFixed(2),
      );
      const b4Sum = Number(
        (
          (metrics.blockAttendance[4] || 0) +
          (metrics.blockSermonScores[4] || 0)
        ).toFixed(2),
      );
      const b5Sum = Number(
        (
          (metrics.blockAttendance[5] || 0) +
          (metrics.blockSermonScores[5] || 0)
        ).toFixed(2),
      );
    ...
    // Data for Sheet 3: Daily Morning Attendance
    const roomMorningDates = Array.from(
      new Set(
        attendance
          .filter(
            (a) =>
              a.type === "MORNING" &&
              roomStudents.some((s) => String(s.id) === String(a.studentId)),
          )
          .map((a) => a.date),
      ),
    ).sort() as string[];

    const dailyAttendanceData = roomStudents.map((s, idx) => {
      const rowData: any = {
        ลำดับ: idx + 1,
        "ชื่อ-นามสกุล": s.name,
        รหัสประจำตัว: s.studentId,
      ...
      const studentAtt = attendance.filter(
        (a) => String(a.studentId) === String(s.id) && a.type === "MORNING",
      );

      roomMorningDates.forEach((date) => {
        const record = studentAtt.find((a) => a.date === date);
        rowData[date] = record
          ? STATUS_LABELS[record.status as any]?.label || record.status
          : "-";
      });
    ...
    if (dailyAttendanceData.length > 0) {
      const wsDaily = XLSX.utils.json_to_sheet(dailyAttendanceData);
      const dailyCols = [
        { wch: 8 }, // ลำดับ
        { wch: 25 }, // ชื่อ-นามสกุล
        { wch: 15 }, // รหัสประจำตัว
        ...roomMorningDates.map(() => ({ wch: 12 })), // dates
      ];
      wsDaily["!cols"] = dailyCols;
      XLSX.utils.book_append_sheet(wb, wsDaily, "รายละเอียดการเข้าแถวรายวัน");
    }

    XLSX.writeFile(
      wb,
      `รายงานสรุป_${reportLevel}_${reportDept}_ห้อง${reportRoom}.xlsx`,
    );
  };
    ...
    <div className="space-y-8 animate-in fade-in duration-500 pb-20">
        ...
        <div className="flex bg-white p-2 rounded-2xl border border-slate-200 shadow-sm w-fit">
          {!isParent && (
            <button
              ...
              className={`flex items-center gap-2 px-8 py-3.5 rounded-xl text-sm font-black transition-all font-heading ${activeTab === "room" ? "bg-slate-900 text-white shadow-lg" : "text-slate-500 hover:bg-slate-50"}`}
            >
          ...
          <button
            ...
            className={`flex items-center gap-2 px-8 py-3.5 rounded-xl text-sm font-black transition-all font-heading ${activeTab === "individual" ? "bg-slate-900 text-white shadow-lg" : "text-slate-500 hover:bg-slate-50"}`}
          >
            <User size={18} />{" "}
            {isParent ? "ผลการเรียนและพฤติกรรม" : "รายงานผลรายบุคคล"}
          </button>
        </div>
        ...
        <div className="space-y-8 animate-in slide-in-from-bottom-4">
          <div className="bg-white p-8 rounded-[32px] border border-slate-200 shadow-sm grid grid-cols-1 md:grid-cols-4 gap-6 items-end">
            <div className="space-y-2">
              <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest ml-1 font-heading">
                ระดับชั้น
              </label>
              <select
                value={reportLevel}
                onChange={(e) => setReportLevel(e.target.value)}
                className="w-full bg-slate-50 border border-slate-200 rounded-xl px-5 py-3.5 font-bold text-slate-900 outline-none"
              >
                ...
                <tbody className="divide-y divide-slate-100">
                  {roomStudents.map((s, idx) => {
                    ...
                    return (
                      ...
                      >
                        <td className="px-8 py-6 text-sm font-black text-slate-300 group-hover:text-slate-900">
                          {idx + 1}
                        </td>
                        ...
                        <td className="px-8 py-6">
                          <span
                            className={`px-3 py-1 rounded-lg text-[10px] font-black uppercase ${s.behaviorScore < 70 ? "bg-red-50 text-red-600" : "bg-green-50 text-green-600"}`}
                          >
                        ...
                        <td className="px-8 py-6">
                          <span
                            className={`px-3 py-1 rounded-lg text-[10px] font-black uppercase ${metrics.englishScore < 5 ? "bg-amber-50 text-amber-600" : "bg-indigo-50 text-indigo-600"}`}
                          >
                        ...
                        <td className="px-8 py-6">
                          <div className="flex items-center gap-3">
                            <div className="flex-1 h-2 bg-slate-100 rounded-full w-24 overflow-hidden shadow-inner">
                              <div
                                className={`h-full rounded-full transition-all duration-1000 ${metrics.rate < 80 ? "bg-amber-500" : "bg-blue-600"}`}
                        ...
                        <td className="px-8 py-6 text-right">
                          <span
                            className={`text-[10px] font-black uppercase ${metrics.remaining > 0 ? "text-red-600" : "text-green-600"}`}
                          >
                            {metrics.remaining > 0
                              ? `ค้าง ${metrics.remaining.toLocaleString()} ฿`
                              : "ครบแล้ว"}
                          </span>
                        </td>
                      </tr>
      ...
      {activeTab === "individual" && (
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-8 animate-in slide-in-from-right-4">
              ...
              <div className="bg-white p-6 rounded-[32px] border border-slate-200 shadow-sm h-[700px] flex flex-col">
                <div className="relative mb-6">
                  ...
                  <input
                    ...
                    className="w-full bg-slate-50 border border-slate-100 rounded-2xl pl-12 pr-4 py-3.5 text-sm font-bold outline-none focus:border-blue-400 transition-all"
                  />
                </div>
                  ...
                  {students
                    ...
                    .map((s) => (
                      <button
                        ...
                        className={`w-full text-left p-5 rounded-2xl border-2 transition-all ${selectedStudentId === s.id ? "bg-slate-900 border-slate-900 text-white shadow-xl scale-[1.02]" : "bg-white border-transparent hover:bg-slate-50"}`}
                      >
                        <p className="font-black text-sm truncate">{s.name}</p>
                        <p
                          className={`text-[9px] font-bold uppercase tracking-widest mt-1.5 ${selectedStudentId === s.id ? "text-slate-400" : "text-slate-500"}`}
                        >
                          {s.studentId}
                        </p>
                      </button>
          ...
          <div
            className={`${isParent ? "lg:col-span-4" : "lg:col-span-3"} space-y-8`}
          >
            {selectedStudent ? (
                ...
                <div className="bg-white p-8 md:p-12 rounded-[48px] border border-slate-200 shadow-sm relative overflow-hidden">
                  ...
                  <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-8 relative z-10">
                    <div className="flex items-center gap-8">
                      ...
                      <div className="space-y-1.5">
                        ...
                        <div className="flex items-center gap-3">
                          <span className="px-3 py-1 bg-blue-600 text-white rounded-lg text-[10px] font-black uppercase tracking-widest shadow-md shadow-blue-100">
                            {selectedStudent.studentId}
                          </span>
                          <p className="text-slate-400 font-bold uppercase text-[11px] tracking-widest">
                            {selectedStudent.level} • แผนก
                            {selectedStudent.department} • ห้อง{" "}
                  ...
                  <div className="grid grid-cols-2 lg:grid-cols-6 gap-6 mt-14">
                    <div className="bg-slate-50 p-6 rounded-[36px] border border-slate-100 text-center hover:bg-white hover:border-blue-400 transition-all group">
                      ...
                      <p
                        className={`text-4xl font-black font-heading ${selectedStudent.behaviorScore < 70 ? "text-red-600" : "text-slate-900"}`}
                      >
                    ...
                    <div className="bg-slate-50 p-6 rounded-[36px] border border-slate-100 text-center hover:bg-white hover:border-blue-400 transition-all group">
                      ...
                      <p className="text-4xl font-black text-blue-600 font-heading">
                        {getStudentMetrics(selectedStudent.id).rate}%
                      </p>
                      <p className="text-[9px] font-bold text-slate-400 uppercase mt-2">
                        ประเมินจากกิจกรรมหน้าเสาธง
                      </p>
                    </div>
                    <div className="bg-slate-50 p-6 rounded-[36px] border border-slate-100 text-center hover:bg-white hover:border-blue-400 transition-all group">
                      <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2">
                      ...
                      <p className="text-4xl font-black text-slate-900 font-heading">
                        {getStudentMetrics(selectedStudent.id).presents}
                      </p>
                      <p className="text-[9px] font-bold text-slate-400 uppercase mt-2">
                        จำนวนวันที่มาเช็คชื่อปกติ
                      </p>
                    </div>
                    <div className="bg-slate-50 p-6 rounded-[36px] border border-slate-100 text-center hover:bg-white hover:border-blue-400 transition-all group">
                      <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2">
                      ...
                      <p className="text-4xl font-black text-indigo-600 font-heading">
                        {getStudentMetrics(selectedStudent.id).sermonScore}
                      </p>
                      <p className="text-[9px] font-bold text-slate-400 uppercase mt-2">
                        บันทึกสมุดโอวาท (เต็ม 25)
                      </p>
                    </div>
                    <div className="bg-slate-50 p-6 rounded-[36px] border border-slate-100 text-center hover:bg-white hover:border-blue-400 transition-all group">
                      <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2">
                      ...
                      <p className="text-4xl font-black text-amber-500 font-heading">
                        {getStudentMetrics(selectedStudent.id).englishScore}
                      </p>
                      <p className="text-[9px] font-bold text-slate-400 uppercase mt-2">
                        คะแนนเต็ม 10
                      </p>
                    ...
                    <div className="bg-slate-50 p-6 rounded-[36px] border border-slate-100 text-center hover:bg-white hover:border-blue-400 transition-all group">
                      ...
                      <p
                        className={`text-4xl font-black font-heading ${getStudentMetrics(selectedStudent.id).remaining > 0 ? "text-red-600" : "text-green-600"}`}
                      >
                        {getStudentMetrics(
                          selectedStudent.id,
                      ...
                      <p className="text-[9px] font-bold text-slate-400 uppercase mt-2">
                        {getStudentMetrics(selectedStudent.id).remaining > 0
                          ? "มียอดค้างชำระ"
                          : "ชำระครบถ้วนแล้ว"}
                      </p>
                    </div>
                  ...
                  <div className="mt-8 bg-slate-50 p-8 rounded-[40px] border border-slate-100">
                    <h4 className="text-xs font-black text-slate-900 uppercase tracking-widest mb-6 flex items-center gap-2">
                      ...
                      รายละเอียดคะแนนเข้าแถวแยกรายบล็อก (เต็ม 5)
                    </h4>
                    <div className="grid grid-cols-5 gap-4">
                      {studyBlocks.map((block) => (
                        ...
                        >
                          ...
                          <p
                            className={`text-xl font-black font-heading ${getStudentMetrics(selectedStudent.id).blockAttendance[block.id] < 4 ? "text-amber-500" : "text-blue-600"}`}
                          >
                            {getStudentMetrics(selectedStudent.id)
                              .blockAttendance[block.id] || 0}
                          </p>
                        </div>
                      ))}
                    </div>
                  ...
                  <div className="mt-8 bg-slate-50 p-8 rounded-[40px] border border-slate-100">
                    <h4 className="text-xs font-black text-slate-900 uppercase tracking-widest mb-6 flex items-center gap-2">
                      ...
                      รายละเอียดคะแนนโอวาทแยกรายบล็อก (บล็อกละ 5 คะแนน)
                    </h4>
                    <div className="grid grid-cols-5 gap-4">
                      {[1, 2, 3, 4, 5].map((block) => (
                        <div
                    ...
                    <div className="space-y-6 relative before:absolute before:inset-y-0 before:left-[19px] before:w-0.5 before:bg-slate-100">
                      {attendance
                        ...
                        .map((a, i) => (
                          <div
                            key={a.id}
                            className="relative pl-12 animate-in slide-in-from-left duration-500"
                            style={{ animationDelay: `${i * 100}ms` }}
                          >
                            <div
                              className={`absolute left-0 top-1.5 w-10 h-10 rounded-2xl border-4 border-white shadow-lg flex items-center justify-center text-white ${a.status === "PRESENT" ? "bg-green-500" : a.status === "LATE" ? "bg-amber-500" : "bg-red-500"}`}
                            >
                              {a.status === "PRESENT" ? (
                            ...
                            <div className="p-5 bg-slate-50 rounded-3xl border border-slate-100 hover:bg-white hover:shadow-md transition-all group">
                              ...
                              <div className="flex items-center gap-2 text-[10px] font-bold text-slate-400">
                                <Clock size={12} /> บันทึกเมื่อ{" "}
                                {a.timestamp.split(" ")[1]} น.
                                {a.remark && (
                      ...
                      {attendance.filter(
                        (a) =>
                          String(a.studentId) === String(selectedStudent.id) &&
                          a.type === "MORNING",
                      ).length === 0 && (
                        <div className="py-16 text-center text-slate-300">
                          <Sun size={64} className="mx-auto mb-4 opacity-10" />
                          <p className="text-sm font-black uppercase tracking-widest">
                            ยังไม่มีข้อมูลการเช็คชื่อเข้าแถว
                          </p>
                        </div>
                      )}
                    </div>
                  ...
                  <div className="bg-white p-8 md:p-10 rounded-[48px] border border-slate-200 shadow-sm h-fit">
                    ...
                    <div className="space-y-4">
                      {behavior
                        .filter(
                          (b) =>
                            String(b.studentId) === String(selectedStudent.id),
                        )
                        .slice(-7)
                        .reverse()
                        .map((b, i) => (
                          <div
                            key={b.id}
                            className="p-6 bg-slate-50 rounded-[32px] border border-slate-100 flex items-center justify-between hover:bg-white hover:shadow-lg transition-all animate-in slide-in-from-right duration-500"
                            style={{ animationDelay: `${i * 100}ms` }}
                          >
                            <div className="flex items-center gap-5">
                              <div
                                className={`w-14 h-14 rounded-2xl flex items-center justify-center shadow-inner ${b.type === "ADD" ? "bg-green-100 text-green-600" : "bg-red-100 text-red-600"}`}
                              >
                                {b.type === "ADD" ? (
                                  <TrendingUp size={24} />
                                ) : (
                              ...
                              <div>
                                <p className="font-black text-sm text-slate-900 leading-tight mb-1">
                                  {b.reason}
                                </p>
                                <div className="flex items-center gap-3 text-[10px] font-bold text-slate-400 uppercase tracking-widest">
                                  ...
                                  <span>
                                    ครู{" "}
                                    {b.recordedBy.split(" ")[1] || b.recordedBy}
                                  </span>
                                </div>
                              </div>
                            </div>
                            <div
                              className={`text-2xl font-black font-heading ${b.type === "ADD" ? "text-green-600" : "text-red-600"}`}
                            >
                              {b.type === "ADD" ? "+" : "-"}
                              {b.score}
                            </div>
                      ...
                      {behavior.filter(
                        ...
                        <div className="py-20 text-center text-slate-300">
                          ...
                          />
                          <p className="text-sm font-black uppercase tracking-widest">
                            ยังไม่มีบันทึกพฤติกรรมในเทอมนี้
                          </p>
                        </div>
                      )}
                    </div>
                ...
                <div className="bg-slate-900 p-10 md:p-14 rounded-[56px] shadow-2xl relative overflow-hidden text-white">
                  ...
                  <div className="flex flex-col lg:flex-row gap-12 items-center relative z-10">
                    <div className="lg:w-1/2 space-y-6">
                      ...
                      <h3 className="text-4xl font-black font-heading tracking-tight leading-none">
                        สรุปยอดเงินและ
                        <br />
                        สถานะการชำระเงิน
                      </h3>
                      <p className="text-slate-400 font-bold text-sm leading-relaxed">
                        ข้อมูลการเงินของบุตรหลานจะถูกอัปเดตทุกครั้งที่มีการชำระเงิน
                        หากมีข้อสงสัยกรุณาติดต่อแผนกการเงินของวิทยาลัย
                      </p>
                    </div>
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <div className="bg-white/5 p-8 rounded-[40px] border border-white/10">
                          ...
                          <div className="flex items-baseline gap-2">
                            <span className="text-3xl font-black text-green-400 font-heading">
                              {getStudentMetrics(
                                selectedStudent.id,
                              ).paid.toLocaleString()}
                            </span>
                        ...
                        <div className="bg-white/5 p-8 rounded-[40px] border border-white/10">
                          ...
                          <div className="flex items-baseline gap-2">
                            <span className="text-3xl font-black text-red-400 font-heading">
                              {getStudentMetrics(
                                selectedStudent.id,
                              ).remaining.toLocaleString()}
                            </span>
                        ...
                        <div className="bg-white/10 p-8 rounded-[40px] border border-white/20 col-span-1 sm:col-span-2 flex items-center justify-between">
                          ...
                          <button
                            ...
                            className="px-6 py-3 bg-white text-slate-900 rounded-2xl font-black text-[10px] uppercase tracking-widest hover:bg-blue-600 hover:text-white transition-all"
                          >
                            ตรวจสอบรายการ
                          </button>
                        </div>
              ...
              <div className="h-[600px] bg-white rounded-[64px] border-4 border-dashed border-slate-100 flex flex-col items-center justify-center p-20 text-center">
                ...
                <h3 className="text-2xl font-black text-slate-900 font-heading uppercase tracking-widest">
                  โปรดเลือกนักเรียนที่ต้องการดูข้อมูล
                </h3>
                <p className="text-slate-400 text-sm mt-3 font-bold opacity-70">
                  เลือกรายชื่อทางด้านซ้ายเพื่อเปิดดูสรุปรายงานผลพฤติกรรมและการเรียน
                </p>
              </div>
            )}
          </div>
